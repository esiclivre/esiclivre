...
class Solicitacao {
	...
	//--------CONSTRUTOR---------------------------
	function __construct($idsolicitacao=null)
	{
	...
	//tipomsg    -> identifica o tipo de mensagem: [M]ovimenta��o - [N]ova solicitação - [R]ecurso
			...
			if(!empty($email))
			{
				...
				//if ($demanda->sistemaOrigem == "1") {//
					...
					else if($tipomsg == "P")
					{
						...
						$body	= "Prezado(a) $nome,<br> <br>
								O atendimento a sua solicitação de informação <b>$processo</b> foi prorrogado, data de previs�o de resposta: $dataprevisaoresposta.
								<br><br>Para acompanhar o andamento, acesse o sistema ".SISTEMA_NOME." no endereço: <a href='".SITELNK."'>".SITELNK."</a>.
								<br>Mensagem Automática do Sistema.";
					}
				...
				/*{
					...
					if($tipomsg == "N")
					{
						...
						$body	= "Prezado(a) $nome,<br><br>
								Sua solicitação de informação <b>$processo</b> foi cadastrada com sucesso.<br>
								Data prevista para resposta: $dataprevisaoresposta.
								<br><br>Para mais informa��es consulte o <a href='".URL_SISTEMA_NOME."'>Sistema de Ouvidoria</a>.
					...
					else if($tipomsg == "P")
					{
						...
						$body	= "Prezado(a) $nome,<br><br>
								Sua demanda de ouvidoria <b>$processo</b> teve prazo foi prorrogado, data de previs�o de resposta: $dataprevisaoresposta.
								<br><br>Para mais informa��es consulte o <a href='".URL_SISTEMA_NOME."'>Sistema de Ouvidoria</a>.
								<br>Mensagem Automática do Sistema ".SISTEMA_NOME;
					}
					if (!sendmail($email,$titulo,$body))
	...
	public function cadastraRecurso($idtiposolicitacao)
	{
        ...
        static public function movimenta($idsolicitacao, $secretariadestino, $despacho, $arquivo)
        {
			...
			if($row["tot"] > 0)
			{
				...
				//Tamb�m não permite movimentar se o SIC de destino for diferente do SIC do usu�rio logado.
				$sql = "select
        ...
        if (!$resultado) {
          return "A demanda não pode ser movimentada. Ocorreu um erro desconhecido.";
        }
        ...
        static public function recebe($idsolicitacao)
        {
			...
			//se a situa��o for aberta (não houve tramita��o), da o recebimento inicial da solicitação
			if ($situacao == "A")
			{
				$sql = "UPDATE
							lda_solicitacao
        ...
        $resultado = execQuery($sql);
        if (!$resultado) {
          return 'Erro na busca por recebimento';
        }
				$row = mysqli_fetch_array($resultado);
				...
				if($row["tot"] > 0)
				{
					//verifica se o usuario � do SIC de destino
        ...
        public static function getPodeRecurso($idsolicitacao, $idsolicitacaoorigem)
        {
            ...
            //se não houver solicitação seguinte, � a de ultima instancia, portanto não cabe mais recurso
	...
	static public function prorrogar($idsolicitacao,$motivo)
	{
                ...
                //se não for prorroga��o de primeira instancia
                if($row['instancia'] != "I")
		...
		if (!execQuery($sql))
                {
                    return "Erro na prorroga��o da solicitação";
                }
                else
                {
                    //envia email de aviso de cadastro de solicitação ao solicitante
