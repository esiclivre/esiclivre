...
function getMenu() {
  ...
  if ($rs) {
    while($row = mysqli_fetch_array($rs)){
      ...
      if ($agrupaMenu != $row['menu'] && $menuTmp) {
...
function getMenuA($menuTmp) {
	...
	if ($count == 1) {
		...
		$menu .= '<li><a href="?'.$menuTmp[0][1].'" class="waves-effect"><i class="material-icons">'.$menuTmp[0][1].'</i>' . $menuTmp[0][0] .'</a></li>';

	} else if ($count > 1) {

		$menu .= '<li class="dropdown"><a href="#" class="waves-effect"><i class="material-icons">'.$menuTmp[0][1].'</i>' . $menuTmp[0][0] .'</a><ul>';
