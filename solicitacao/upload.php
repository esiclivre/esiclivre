<?php
  ...
  $arqName = $arqType = $arqSize = $arqTemp = $arqError = null;
...
if (isset($_POST['acao'])) { //inicio if post
...
function enviadados(){
	...
	//###################################
		...
		$row = mysqli_fetch_array($sql);
		$idsolicitacao = $row['idsolicitacao']; //ida da solicita��o para nome do arquivo
    ...
    $arq_ext = explode('.', $arqName);
    $extensao = strtolower(end($arq_ext));
