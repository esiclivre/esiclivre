...
class Solicitation 
{
	...
	public function cadastra()
	{
		if($this->validaDados())
		{
			...
			if (!mysqli_query($con, $sql)) {
				$this->erro = "Erro ao inserir solicitação ". mysqli_error($con);
				$sucesso = false;
			} else {
				$this->idsolicitacao = mysqli_insert_id($con);
                                ...
                                if(!empty($this->idsecretariaselecionada))
                                {
                                    ...
                                    self::enviaEmailSic($rec['idsecretaria'],"N");
                                }
                                else
                                {
                                ...
                                self::enviaEmailSolicitante($this->idsolicitacao,"N");
	...
	public function cadastraRecurso($idtiposolicitacao)
	{
		...
		{
                        $configuracao = self::getParametrosConfiguracao();
			...
			else
			{
				...
				$sucesso = true;
                                ...
                                //envia email de aviso de cadastro de solicitação ao solicitante
                                self::enviaEmailSolicitante($this->idsolicitacaoorigem,"N");
		...
		if (!$con->query($sql))
                ...
                else
                {
                    ...
                    self::enviaEmailSic($secretariadestino,"M");
	...
	static public function finaliza($idsolicitacao,$tiporesposta,$resposta, $arquivos)
	{
		...
		if (!$all_query_ok)
                ...
                else
                {
                    ...
                    //envia email de aviso de cadastro de solicitação ao solicitante
                    self::enviaEmailSolicitante($idsolicitacao,"R");
	...
	static public function prorrogar($idsolicitacao,$motivo)
	{
		...
		if (!execQuery($sql))
                ...
                {
                    //envia email de aviso de cadastro de solicitação ao solicitante
                    self::enviaEmailSolicitante($idsolicitacao,"P");
