...
class Solicitante {
	...
	function validaDados()
	{
		...
		//solu��o adotada remover verifica�oes de senha
		//essas verifica��es impedem que usuario faca alteracoes no cadastro
		/*------------------------------------------------------------------
		if (empty($this->senha))
		{
		...
		{
			$this->erro = "Confirma��o da senha não informada";
			return false;
		}
		...
		if (mysqli_num_rows($rs)>0)
		{
			...
			if ($con->query($sql))
			{
				...
				$body="Prezado(a) $this->nome,<br> <br>
                                        ...
                                        **A senha de acesso à aquela informada no cadastro. Caso não se lembre, solicite o envio de uma nova senha pelo link \"Esqueci a senha\" no formulário de login do sistema.
                                        ";
