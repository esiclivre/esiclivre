...
class Solicitante {
	...
	function __construct($idsolicitante=null)
	{
	...
	function cadastra()
	{
        ...
        public function reenvioConfirmacao(){
                ...
                $body="Prezado(a) ".$row['nome'].",<br> <br>
                        ...
                        Você precisa completar seu cadastro para ter acesso ao sistema ".SISTEMA_NOME.". Para isso, precisamos da sua confirmação, favor acesse o seguinte endere�o: <br/><br>
	...
	{
		$sql = "select idsolicitante, nome, email, telefone,tipopessoa, cpfcnpj from lda_solicitante where md5(idsolicitante) = '$idsolicitantecripto' and dataconfirmacao = 0";
		$rs = execQuery($sql);
		...
		if (mysqli_num_rows($rs)>0)
		{
			...
			if ($con->query($sql))
			{
				...
				$body="Prezado(a) $this->nome,<br> <br>
                                        ...
                                        **A senha de acesso é aquela informada no cadastro. Caso n�o se lembre, solicite o envio de uma nova senha pelo link \"Esqueci a senha\" no formul�rio de login do sistema.
                                        ";
	...
	function resetaSenha($cpfcnpj)
	{
		...
		if (mysqli_num_rows($rs) > 0)
		{
			...
			else
			{
				...
				if (!sendmail($email,'Solicitaçao de confirmação de cadastro',$body))
				{
					$this->erro = "Ocorreu um erro no envio do e-mail. Favor tente mais tarde.";
					return false;
				}
